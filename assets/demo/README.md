...
The project owner explicitly requested public deployment and repository walkthroughs in this session. The butterfly art is user-supplied; this publication instruction does not establish or grant a general third-party reuse license. Font copyright and OFL texts remain under the demo's `assets/fonts/` and `public/font-licenses/` directories. Research and review status is not changed by the recording.

两个动图分别使用英文和中文界面，展示真实网页操作。网站发布不等于将蝴蝶素材或录屏中的第三方素材另行授予 MIT 许可。
